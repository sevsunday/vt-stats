...
34. **Final ingest run (deliverable): the three 08-23 matches go live.** With the parity audit green and the v3 files back in `data/sessions/VTrider/`, run `python scripts/process_stats.py` (incremental — only the 3 new files process; the parity run already cached the corpus at PIPELINE_VERSION 27). Expected end state: `data/processed/2026-08-23T*.json` ×3; manifest grows to N+3 with the three as `manifest[0..2]` (newest-first — `2026-08-23T02-50-32` becomes the default landing match); `match_contributions.json` +3 entries; ELO pairs (canonical + thugs_only) gain 3 rated matches (`match_count` +3 — all three pass the 6-player/240s gates at 8 players and 14–37 min; ratings and `peak_vtsr` legitimately shift); `map_stats.json` entries for vsrscammed / STBarrenvsr / vsrebola (registry fetch may pull one new map image if vsrscammed is uncataloged — idempotent, filename fallback otherwise); player-page stubs regenerate for the 8 affected players (all existing regulars; sticky slugs unchanged). Optionally rerun `python scripts/validate_elo.py` for a fresh `_validation/` report. Spot-check each match in the dashboard against item 29's acceptance criteria. **No commits** — leave the new raw files (currently untracked) + regenerated outputs for the user to review and commit together (`git add data/sessions/VTrider/2026-08-23-*` + `data/processed/` + stub pages).
