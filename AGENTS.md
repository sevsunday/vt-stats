...
- `scripts/build_map_registry.py` / `data/map-registry.json` / `data/maps/` — build script + generated registry + fetched map assets. Walks distinct maps in `matches.json` and fetches metadata (title, description, top-down image, team names) from `https://gamelistassets.iondriver.com/bzcc/getdata.php` for each. Merges iondriver data with the baked-in `VSR_MAP_DATA` from `js/bz2api.js` (author, canonical size/b2b). Downloads images to `data/maps/<mapFile>.png`. Idempotent: re-runs are zero-network when maps are already cached. Invoked at the end of the main pipeline run. Consumed by `js/app.js` for hero stats + `js/positioning-charts.js` for heatmap overlays.
...
- `scripts/audit_sentinel_events.mjs` — permanent debugging utility that scans every `data/sessions/**/*.binpb.gz` and histograms `DamageDealt` / `DamageReceived` events exceeding the `SENTINEL_DAMAGE_THRESHOLD` (> 1e6). Primary metric `total_sentinel_pairs` must agree with `all_matches.json → meta.total_sentinel_damage_dropped`. Writes reports into gitignored `_investigation/output/`.
...
- `docs/sentinel-damage.md` — first-class reference doc for the `amount > 1e6` sentinel damage filter. Evidence chain, BZCC engine struct layout, Ghidra decompile of the `DAMAGE_TYPE_UNKNOWN` force-kill pathway, bit-pattern verification, and the proposed upstream `DamageType` enum schema enhancement. Linked from `.cursor/rules/data-schema.mdc` and `DEVELOPER_GUIDE.md`.
