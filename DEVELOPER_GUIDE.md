...
One semantic worth internalizing: **a queue row reports the pre-purchase bank.** The engine debits on the tick after a QUEUE (`t=3515` QUEUE, `t=3516` bank 80 → 60), so the number next to an order is what the commander actually had available to spend, while a build row shows the bank at completion. On the reference match the first Collector reads `40 | 40 | 0` at queue and `20 | 40 | 0` at completion, which is the charge landing in between. All three fields are `null` on a build-data-but-no-resource-data session and on any event preceding the first resource tick, where `econBuildChip()` falls back to the bare cost — a missing reading must never render as `0`. And `max_scrap_at_event` is the wire's own cap, already encoding `40·recycler_alive + 20·pools`, so it should never be recomputed from the pool count.
