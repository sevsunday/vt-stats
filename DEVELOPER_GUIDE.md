...
**v4 downstream surfaces** (full reference: `docs/DATA_DICTIONARY.md` §5 `economy` / `builds` + `.cursor/rules/data-schema.mdc`): per-match `economy` + `builds` blocks (match-global, always-unfiltered; `has_resource_data` / `has_build_data` flags with absent-=-false defaults), the per-match **Economy tab**, highlights 12 → 15 (`the_tycoon` / Elon Musk scrap-income + `war_machine` / Conveyor Belt combat-ship value + `first_upgrade` race to first live upgraded pool), the `commander_economy` contribution slice → aggregator career econ rollup → Commanders-tab Economy Leaders card + player-page commander econ block + `career_tycoon` / `career_war_machine` cards, the VTSR-C v2 econ composite (§13.12, inert), raw-browser `buildEvent` arm + reconcile rows, and the first-batch audit runbook `scripts/audit_build_events.mjs`.
