...
- **Ranked ladder** (`ELO_LADDER_MIN_MATCHES = 25` **and** globally active): a player occupies a `#` when `matches_played >= 25` and `inactive_status == "active"` (any corpus `leaderboard[]` or F9-duel appearance within 30 days of the newest corpus match, or 3 games after a gap). Lookup miss → inactive. Everyone else stays visible in a single Unranked table (no rank numbers; chips `Inactive · Nd ago` / `Returning · N of 3` / match-count; never-seen = `Inactive` with no `· 0d ago`). Display-only (`leaderboard_eligible` / top-level `inactivity_window_days` / `comeback_games_required` / `corpus_latest_date`); no `ELO_SCHEMA_VERSION` bump. Helper: `scripts/inactivity.py`. Memos: `critique/decisions/vtsr-t-ladder-eligibility.md`, `critique/decisions/vtsr-inactivity-threshold.md`. Do not retune 25 / 30 / 3 to chase a name. Distinct from the K-factor inactivity boost (`K_INACTIVITY_BOOST_*`). Tools / Balonce consume the rating, not the rank.
...
| `INACTIVITY_WINDOW_DAYS` | 30 | Display-only: any-appearance idle window vs newest corpus match |
...
**v5 — inactivity + commander-stale clocks (display-only; `CMDR_ELO_SCHEMA_VERSION 4 → 5`).** A commander occupies a ranked `#` only when the v4 duel-count OR-gate still holds **and** they are globally active (any corpus `leaderboard[]` or F9-duel appearance within 30 days of the newest corpus match, or 3-game comeback) **and** command-recent (commanded within 90 days — corpus `is_commander` / `team_leaders` or F9 `commanders.{1,2}` — or 3 commander games after going stale while still thugging). Lookup miss → inactive + stale. Ratings, K, and `duels[]` are byte-identical to schema 4 aside from the new fields. Helper: `scripts/inactivity.py`. Memo: `critique/decisions/vtsr-inactivity-threshold.md`. Do not retune 30 / 90 / 3 to chase a name. No `PIPELINE_VERSION` / `ELO_SCHEMA_VERSION` / `match.schema_version` bump.
