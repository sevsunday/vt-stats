...
The same pass reworked the card's controls. The header composes three orthogonal things: a commander-scope group whose labels are rewritten per match from `builds.teams{n}.commander.name` (picking one commander drops the other column and renders the survivor at `col-12`), the durable type-filter pills, and a 120 ms-debounced find bar matching the *displayed* unit name — via a shared `econRowDisplayName()`, so rows whose pipeline `name` is null still match their `odf_map` fallback — plus the ODF stem and the resolved producer lane. Scope and query reset on every match switch since both are about the match that just left the screen; the type filter persists as a durable lens. The input sits in the card header rather than the log body, so re-rendering cannot steal focus mid-keystroke. New build-log helpers belong at or after `buildlogFilterFn`, because `_investigation/check_econ_tooltips.mjs` slices `js/app.js` from `renderEconomyProductionCard` to that marker.
