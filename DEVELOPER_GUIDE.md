...
**Single profile** (pre-gen stub or `player/index.html?p=<steam64>` / `?slug=<slug>`): six tabs. The address bar tracks the open pill: Overview omits `tab`, and the others are `?tab=rating|axes|highlights|rivals|loadout` (a redundant `overview` or unknown slug is stripped). Pill clicks `pushState`, so Back returns to the previous pill. The same query works on `/player/<slug>/`.
