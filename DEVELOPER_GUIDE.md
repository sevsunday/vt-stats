...
**Consumers:** ELO page **VTSR-C Elo pill** (`#elo-tab-vtsr-c` — the `#section-vtsr-c` ladder card, table-only; `?tab=vtsr-c`), How-it-works (the `commanderLadderHtml()` section from `js/vtsr-explainers.js`, which is where the ladder card's "How It's Calculated" button lands), Commanders & fairness (two cards), Does-it-work (accuracy cards + λ table + axis-outcome bar list); dashboard Commander Cohort card (top-5 VTSR-C strip via lazy `ensureCommanderEloLoaded()`); player page Rivals tab (**Commander Rivalries** panel — headline strip, top opponent, top-5 enemies, nemesis/best chips; opponent tallies computed client-side in `js/player.js` from the contributions walk because the aggregator's `head_to_head` is capped at top-10 globally). All consumers are 404-safe (missing files → surfaces hide / em-dash).
