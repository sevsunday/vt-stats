...
- **Ranked ladder** (`ELO_LADDER_MIN_MATCHES = 25`): a player occupies a `#` when `matches_played >= 25`. Everyone else stays visible in a single Unranked table (no rank numbers; match-count chip, no `/ 25` printed on the chip). Display-only (`leaderboard_eligible` / top-level `leaderboard_min_matches`); no `ELO_SCHEMA_VERSION` bump. Memo: `critique/decisions/vtsr-t-ladder-eligibility.md`. Do not retune 25 to chase a name. Tools / Balonce consume the rating, not the rank.
...
- **VTSR-T is untouched** — `scripts/elo.py` never reads the ledger; `elo_history.json` hashes byte-identical across the import. Gate: `_investigation/check_f9_ledger.py` (funnel partition, duel shape, alias validity, forbidden-consumer scan, ladder cross-checks).
