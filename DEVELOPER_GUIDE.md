...
**v4 downstream surfaces** (full reference: `docs/DATA_DICTIONARY.md` §5 `economy` / `builds` + `.cursor/rules/data-schema.mdc`): per-match `economy` + `builds` blocks (match-global, always-unfiltered; `has_resource_data` / `has_build_data` flags with absent-=-false defaults), the per-match **Economy tab**, highlights 12 → 16 (`the_tycoon` / Elon Musk scrap-income + `loose_collector` / Loose Collector + `war_machine` / Conveyor Belt combat-ship value + `first_upgrade` race to first live upgraded pool), the `commander_economy` contribution slice → aggregator career econ rollup → Commanders-tab Economy Leaders card + player-page commander econ block + `career_tycoon` / `career_loose_collector` / `career_war_machine` cards, the VTSR-C v2 econ composite (§13.12, inert), raw-browser `buildEvent` arm + reconcile rows, and the first-batch audit runbook `scripts/audit_build_events.mjs`.

**Thug supply + Economy card v2 (`match.schema_version` 18, `PIPELINE_VERSION` 33).** Purely additive telemetry, rating-inert by construction and by gate (`_investigation/golden_v18_no_drift.py` strips every v18 field from the corpus and requires byte-identical VTSR-T and VTSR-C output). Three parts: (1) a new **always-present** top-level `thug_supply` block — per-team `thug_ships_lost` / `thug_ship_value_lost` and `thug_pilot_sec` / `thug_at_base_pilot_sec` / `reship_median_sec` / `reship_spells` (positioning). It is **corpus-wide, not v4-gated**, since none of its inputs need proto v4; the two halves gate independently and null out rather than reporting a fake zero. (2) new `economy.teams` fields `peak_max_scrap`, `green_sec` / `yellow_sec` / `red_sec` and `upgrades_built`. (3) new positioning metrics `pilot_sec`, `reship_median_sec`, `reship_spells`.
...
**Build-feed economic context (`match.schema_version` 21, `PIPELINE_VERSION` 36).** Every `builds.feed[]` row now reports the state of its team's economy at its own tick: `scrap_at_event`, `max_scrap_at_event` and `pool_count_at_event`. It is purely additive display telemetry behind the Build Order Log's four-segment badge (`cost | bank | cap | pools`), and it needed no new gate — `golden_builds_inert.py` already blanks the entire feed, so the rating stack demonstrably cannot see any of it.

The values ride the same "latest wire value" join that has always stamped `scrap_status_at_*`: `econ_state[side]` gained `cur_bank` / `cur_cap` / `cur_pools` next to `cur_status`, written on every full-rate resource tick and read when the BuildEvent row is constructed. The income classifier's `prev_bank` / `prev_max_scrap` / `prev_pools` keys hold the same numbers at rest, but they are named for a per-tick delta comparison and are assigned partway through that comparison, so overloading them would invite a reordering bug for no gain. Because `UpdateTick` carries `ResourceState` every tick, the reading is exact to 0.05 s — `_investigation/probe_feed_context.py` replays the wire and confirms all three fields on rows sampled across the reference match with zero mismatches.

One semantic worth internalizing: **a queue row reports the pre-purchase bank.** The engine debits on the tick after a QUEUE (`t=3515` QUEUE, `t=3516` bank 80 → 60), so the number next to an order is what the commander actually had available to spend. **v24 (`match.schema_version` 24, `PIPELINE_VERSION` 42):** a matched BUILD copies that QUEUE-time triad onto the completion row so the chip is about the order, not the bank when the unit rolled off the line (orphan BUILD keeps the completion stamp; CANCEL stays on its own tick). The same bump adds `builds.teams.{n}.scavs_built` (mobile scavenger BUILD completions — Scavenger / Collector / Harvester) and `thug_supply.{n}.commander_row` (commander attrition sibling; team totals stay thug-only). Both are display-only and stay out of `_extract_contribution`. On a build-data-but-no-resource-data session and on any event preceding the first resource tick, all three context fields are `null` and `econBuildChip()` falls back to the bare cost — a missing reading must never render as `0`. And `max_scrap_at_event` is the wire's own cap, already encoding `40·recycler_alive + 20·pools`, so it should never be recomputed from the pool count.

The same pass reworked the card's controls. The header composes three orthogonal things: a commander-scope group whose labels are rewritten per match from `builds.teams{n}.commander.name` (picking one commander drops the other column and renders the survivor at `col-12`; radio, one selection at a time), the durable type+kind filter pills, and a 120 ms-debounced find bar matching the *displayed* unit name — via a shared `econRowDisplayName()`, so rows whose pipeline `name` is null still match their `odf_map` fallback — plus the ODF stem and the resolved producer lane. Type pills (Queues / Builds / Cancels) and kind pills (Combat ships / Structures) are independent checkbox axes ANDed together (empty axis = any); **All events** is an exclusive reset that clears every other pill. Combat ships is the pipeline `combat_ship_odfs` set minus constructor-lane rows (same set as `ships_built` / Conveyor Belt); Structures stays `producer === constructor`. The stem list is the tiny sidecar `data/combat_ship_odfs.json` (`schema_version: 1`), emitted by `write_combat_ship_odfs()` next to the existing `Combat-ship set:` log so ODF DB updates stay in sync — the dashboard lazy-fetches it rather than loading `odf.min.json`. Scope and query reset on every match switch since both are about the match that just left the screen; the type+kind filter persists as a durable lens. The input sits in the card header rather than the log body, so re-rendering cannot steal focus mid-keystroke. New build-log helpers belong at or after `buildlogFilterFn`, because `_investigation/check_econ_tooltips.mjs` slices `js/app.js` from `renderEconomyProductionCard` to that marker.
...
- **v3/v4**: the maps are `reserved`; working identity is the valid-Steam64 **union of `UpdateTick.players`** via `_build_identity_maps()` in [scripts/process_stats.py](scripts/process_stats.py). `header.players` is a nickname / first-seen-slot hint that is **always compared** (`match.roster_qa`; not gated on header length). The shim (a) drops entries failing the `_valid_steam64` gate (`(s64 >> 32) == 0x01100001` — the v3 collector emits garbage Steam64s for EMPTY slots because it scans without a player-handle guard), (b) drops header-only 0-tick ghosts (`reason: "no_tick_presence"` — Sept 7 2026 collector leak) and synthesizes tick-only ids (empty header / missed late joiners), (c) places every **evidenced** player at the majority-vote slot from the ENGINE's own event attributions (the `team` fields on `DamageDealt` / `UnitDestroyed` / `UnitSniped` / `PickupPowerup` are team slots 1-10 stamped per event), and (d) falls back for **no-evidence tick-union** players (campod spectators): roster hint honored when free, overflow to the lowest free same-side slot when taken and UpdateTick presence `>= ROSTER_OVERFLOW_MIN_SHARE = 0.25`, dropped below the floor; silent tick-only with no slot hint drop as `tick_only_no_slot`. Every correction/reassignment/drop is recorded in `match.roster_conflicts` (`resolution`, `reason`, `assigned_slot`, `presence_share`, `evidence_events`). The raw roster (including invalid entries, flagged `valid: false`) is preserved on `match.roster` for provenance.
...
- **`roster_conflicts`** — identity-shim audit (`[]` when header and ticks agree and no slot correction fired). v23 adds `reason`.
- **`roster_qa` (v23)** — always-on header-vs-tick-union QA (`header_valid`, `tick_union`, `ghosts_dropped`, `tick_only_added`, `max_tick_players`, `agreed`). `null` pre-v3. Console stays quiet when `agreed`. Working identity is the tick union; header-only 0-tick ghosts are dropped. Gated by `_investigation/check_roster_tick_union.py`.
...
Per-match award catalog rendered below the Faction Scoreboard. A fixed slate of 12 always-on cards — The Bully, Grim Reaper, Bullet Sponge, The Hustler, Sharpshooter, Gunner, Puppeteer, Frenemies, Roadrunner, Pod Goblin, Chris Kyle, The Locksmith — plus four proto-v4 commander-economy cards (Elon Musk / Loose Collector / Conveyor Belt / First Upgrade) that self-omit pre-v4. Each card emits unconditionally as long as its data gate passes; cards whose underlying data is unavailable (positioning / pickups / T-key gates, missing snipes, no resource/build telemetry, etc.) are simply omitted, and the grid reflows around the gap.

Built by `compute_highlights()` in [scripts/process_stats.py](scripts/process_stats.py) from already-built per-match blocks (`leaderboard`, `top_rivalries`, `pickups`, `powerup_destructions`, `snipes`, `positioning`, `economy`, `builds`). No event re-walking. The block is **match-global and always-unfiltered** — the dashboard reads `currentData.highlights`, never the filtered `data` view, and the renderer is pure formatting.
...
`value_format` drives `formatHighlightValue()` in `js/app.js` — `"damage"`/`"count"`/`"score"`/`"distance"`/`"scrap"` are locale-formatted integers, `"ratio"` and `"kd"` use two decimals, `"percent"`/`"accuracy"` render as `(value * 100).toFixed(1) + '%'`, `"duration"` renders as `Nm:Ss` (e.g. `1m:30s`). The `"kd"` format is functionally identical to `"ratio"` at the value-formatting layer; the distinct enum value tells `formatHighlightBreakdown()` to render `{kills}K / {deaths}D` (with `(perfect)` when `deaths == 0`).
...
1. **The Verdict** — six stat cells with team-colored name attribution and a trophy marker on each stat's winner: scrap income, extractor war, field control (front-line means, em-dash without positioning), materiel lost (all hulls incl. AI-owned — tooltip states the deliberate difference from `thug_supply`'s human-only accounting), structure investment, and result (tooltip explains `decided_by` provenance, mirroring `applyWinnerBadge` copy).
2. **Match Timeline** — four Chart.js lanes sharing one x-domain with a synced crosshair, per-lane explainer lines, glass tooltips, and **x-only drag-zoom synced across all lanes** (vendored `chartjs-plugin-zoom`, mirroring the Economy pools-lane link pattern; shared Reset-zoom button; the scrap-status **band strips** under the Momentum lane re-clip to the zoom window). Lanes: Momentum (net combat-ship value bars + pool-advantage line + beat-flag scatter with native tooltips; gold vertical lines mark weight-5 beats on every lane), Map control (smoothed front lines + base-intruder bars — the rush detector), War machine (fielded/lost cumulatives), Combat intensity.
3. **Key Moments** — the curated beats rail: Bootstrap icons per kind, team chips, detail sub-lines; rows with payloads (kill-burst unit lists) expand via a chevron; recycler/factory destruction rows are title-only (no killing-blow subtext or expander); hovering locates the beat on all lanes via the crosshair; **clicking a row opens the 3D Replay at that moment** through `window.vtOpenReplayAtTick(tick)` (exposed by `app.js`: sets `pendingReplayTick`, clears the pane's rendered flag, shows the tab — the replay iframe reboots with `?t=<sec>`). Same-second `result` beats sort last so a host-attested win sits after the recycler/factory kill that is the win condition.
...
Any combination of match, filter, and tab is shareable via query parameters.

**History.** The first load and `setLiveSync(true)` use `history.replaceState` on the entry you already landed on. After that, a user change to match, tab, or the per-match filter **pushes** when Live Sync is on, so Back and Forward walk those views. `popstate` applies the URL with writes suppressed. Picker facets always write (Live Sync off included) and push the same way; the first committed search query pushes and later commits while that field stays focused replace. Replay fullscreen still pushes a same-URL sentinel, and Back on that entry only exits fullscreen.

**URL writes for match / tab / filter are gated by the Live Sync toggle** (Settings gear, `bi-broadcast`). When OFF (default), `syncUrl()` is a no-op and those clicks leave the URL alone, so Back leaves the page. When ON, every state change writes to the URL. The preference persists in `localStorage` under the `vt-url-sync` key. Share includes the Has VOD facet (`vod`), matching the picker URL.
...
| Input | Behavior |
|---|---|
| Default home visit, Live Sync off | URL stays clean (no params); preloader shows, first match loads silently |
| Default home visit, Live Sync on | First load `replaceState`s `?match=<newest>` onto the landed entry (no extra Back step) |
| Later match / tab / filter change, Live Sync on | `pushState`; Back restores the previous view without reloading past it |
| Picker chip or search | `pushState` even when Live Sync is off (search refines with `replaceState` while the field stays focused) |
| Replay fullscreen, then Back | URL unchanged, so Back only exits fullscreen |
...
**Visibility.** The dashboard's per-match Player Leaderboard still shows excluded rows with `Campod` / `Partial` badges (`.vt-campod-badge` / `.vt-partial-badge`) and 55% opacity (`.vt-row-campod` / `.vt-row-partial`) for transparency — only the rating math skips them. The badges carry hover tooltips reading the underlying `campod_share` / `presence_window_sec` so reviewers can audit the gate's decision. The Faction Scoreboard roster omits `is_campod` players entirely (chips only — team totals, leaderboard, kill feed, and every other surface still include them).
...
- **Excluded** matches (fewer than 6 non-campod leaderboard rows, OR `duration_sec < 240`, OR a host-attested cancellation) do not increment `matches_played` and contribute no deltas to ratings. Camera-pod spectators do not count toward the 6; idle thugs and partials do (they are omitted from the rated lobby, and the match stays rated). Raw `player_count` is the slot count and is not the gate. They appear in `elo_history.json` with `match_excluded: true` and an empty `deltas` array so the exclusion counters reconcile. Cancelled matches additionally carry a never-applied `shadow` block (§13.9.1).
- **Provisional** badge: rated rows with `matches_played < 10` (`ELO_PROVISIONAL_THRESHOLD`) display a `?` chip. Provisional players ARE rated. The chip signals "rating is still moving fast — interpret with caution". Independent of the ranked-ladder gate: a 12-match player has a real tier badge and is still Unranked.
- **Ranked ladder** (`ELO_LADDER_MIN_MATCHES = 25` **and** globally active): a player occupies a `#` when `matches_played >= 25` and `inactive_status == "active"` (any corpus `leaderboard[]` or F9-duel appearance within 90 days of the newest corpus match, or 3 games after a gap). Lookup miss → inactive. Everyone else stays visible in a single Unranked table (no rank numbers; chips `Inactive · Nd ago` / `Returning · N of 3` / match-count; never-seen = `Inactive` with no `· 0d ago`). Display-only (`leaderboard_eligible` / top-level `inactivity_window_days` / `comeback_games_required` / `corpus_latest_date`); no `ELO_SCHEMA_VERSION` bump. Helper: `scripts/inactivity.py`. Memos: `critique/decisions/vtsr-t-ladder-eligibility.md`, `critique/decisions/vtsr-inactivity-threshold.md`. Do not retune 25 / 90 / 3 to chase a name. Distinct from the K-factor inactivity boost (`K_INACTIVITY_BOOST_*`). Tools / Balonce consume the rating, not the rank.
...
**Module:** `scripts/elo_commander.py` (mirrors the `elo.py` contract; the pipeline passes combat-ship stems, so the rating walk does no I/O). `compute_commander_elo(all_match_data, elo_history) -> (current, history)`, wired into `scripts/process_stats.py` immediately after the canonical VTSR-T emit. **Outputs:** `data/processed/elo_commander_current.json` + `elo_commander_history.json` (`schema_version: 6`; ratings comparable with schema 5; both in the `load_cache_index()` skip set). **Corpus-wide, picker-unaware, NOT in the pipeline cache key** (separate ladder from VTSR-T). No `PIPELINE_VERSION` / `ELO_SCHEMA_VERSION` / `match.schema_version` interaction — VTSR-C recomputes every run from in-memory data, exactly like VTSR-T.
...
**v2 (proto v4 overhaul): the economy-performance composite, shipped structurally inert.** Every duel where the match carries BOTH `has_resource_data` and `has_build_data` records an opening economy composite from the `economy` + `builds` blocks (constructor-free by design — era-mixed structure-completion quality must never feed a rating axis): `pool_tempo` (seconds sooner to 3 pools inside the first 240s; later or never censored at 240s — 5 pools is a mid-match event and is not the opening clock), `combat_conversion` (opening combat-ship BUILD scrap / opening 1 Hz income), `regen_tempo` (opening share of alive-recycler samples in the red band). `replacement_ratio`, `upgrade_share`, and `loose_share` (`income_loose / max(scrap_income, 1)`, whole match) stay in the audit at weight 0. Weights `COMMANDER_ECON_WEIGHTS` = pool_tempo .43 / combat_conversion .35 / regen_tempo .22 / the three audit axes 0. **Within-match differential normalization** (n=2 makes lobby z-scores degenerate; the opponent diff controls for map/patch/lobby size): `z = clip(d / shrunk_std(axis), −2, 2) / 2` where `shrunk_std` is a rolling RMS of team-1-perspective diffs with seed prior + shrinkage (`CMDR_ECON_STD_SHRINKAGE = 10` pseudo-duels; snapshot-before-duel / update-after — `commander_shrunk_baseline` mechanics); `P₁ = Σw·z / Σw` over available axes with weight > 0, `P₂ = −P₁`. **Blend** `S' = α_c·S + (1−α_c)·(P+1)/2` applies only on telemetry duels, and the code **branches on `CMDR_ALPHA_C < 1.0`** so the inert path never routes through blend arithmetic (provable by inspection on top of the golden byte-identity gate `_investigation/golden_vtsrc_v2.py` — every duel byte-identical to v1). Fallback duels (all pre-v4 forever; collector gaps) score outcome-pure S at ANY α_c — a determined outcome is always a valid duel signal (RATIFIED 2026-09-03). W/L/D records always tally the RAW outcome. Formulas, seed priors, and the promote rule are FROZEN in `critique/decisions/vtsr-c-v2-composite.md` (2026-09-23 amendment: the pre-amendment corpus is discovery only; flip below 1.0 only on duels dated after that day, with ≥ 25 of them, a Wilson lower bound > 0.55 on ≥ 2 of 3 scored axes, close-game agreement not below 0.50 once that subset has ≥ 15 duels, and log-loss improved by ≥ 0.01). `CMDR_ELO_SCHEMA_VERSION` 6; ratings comparable with schema 5. New v2 fields: top-level `alpha_c` / `econ_weights` / `econ_std_prior` / `econ_std_shrinkage` / `thug_supply_cap` / `econ_std_observed`; per-rating `duels_with_telemetry`; per-duel `performance {available, p, axes{axis: {diff, std, z}}}` + per-side `score_blend {alpha_c, s_raw, s_blended}`.
...
| `CMDR_LADDER_MIN_V4` | 8 | Display-only: proto-v4 games to occupy a ranked `#` |
| `CMDR_LADDER_MIN_NON_V4` | 25 | Display-only: older games (F9 + pre-v4) to occupy a ranked `#` |
| `INACTIVITY_WINDOW_DAYS` | 90 | Display-only: any-appearance idle window vs newest corpus match |
| `COMEBACK_GAMES_REQUIRED` | 3 | Display-only: games (command games on VTSR-C) to rejoin after a gap |
| `CMDR_STALE_WINDOW_DAYS` | 90 | Display-only: commander grace while still thugging |
...
**Consumers:** ELO page **VTSR-C Elo pill** (`#elo-tab-vtsr-c` — the `#section-vtsr-c` ladder card, table-only; `?tab=vtsr-c`; ranked table + one Unranked table), How-it-works (the `commanderLadderHtml()` section from `js/vtsr-explainers.js`, which is where the ladder card's "How It's Calculated" button lands), Commanders & fairness (two cards), Does-it-work (accuracy cards + λ table + axis-outcome bar list); dashboard Commander Cohort card (top-5 **eligible** VTSR-C strip via lazy `ensureCommanderEloLoaded()`); player page **Commander** pill (`?tab=commander` — VTSR-C headline, time-series, opposing commanders, favorite maps, faction when commanding, rated duel log; opponent tallies computed client-side in `js/player.js` from the contributions walk because the aggregator's `head_to_head` is capped at top-10 globally). All consumers are 404-safe (missing files → surfaces hide / em-dash). Tools / Balonce consume the rating, not the rank.

**v5 — inactivity + commander-stale clocks (display-only; `CMDR_ELO_SCHEMA_VERSION 4 → 5`).** A commander occupies a ranked `#` only when the v4 duel-count OR-gate still holds **and** they are globally active (any corpus `leaderboard[]` or F9-duel appearance within 90 days of the newest corpus match, or 3-game comeback) **and** command-recent (commanded within 90 days — corpus `is_commander` / `team_leaders` or F9 `commanders.{1,2}` — or 3 commander games after going stale while still thugging). Lookup miss → inactive + stale. Ratings, K, and `duels[]` are byte-identical to schema 4 aside from the new fields. Helper: `scripts/inactivity.py`. Memo: `critique/decisions/vtsr-inactivity-threshold.md` (global window amended 30 → 90 on 2026-09-24; no schema bump). Do not retune 90 / 3 to chase a name. No `PIPELINE_VERSION` / `ELO_SCHEMA_VERSION` / `match.schema_version` bump.

**v4 — ranked-ladder eligibility (display-only; `CMDR_ELO_SCHEMA_VERSION 3 → 4`).** A commander occupies a ranked `#` when `duels_with_telemetry >= CMDR_LADDER_MIN_V4` (8) **OR** `duels_non_v4 >= CMDR_LADDER_MIN_NON_V4` (25). Older = F9 ledger + pre-v4 corpus (`matches_commanded_rated − duels_with_telemetry`). Ratings, K, and duel history are byte-identical to schema 3 aside from the new fields (`duels_non_v4`, `leaderboard_eligible`, top-level `leaderboard_min_v4` / `leaderboard_min_non_v4`). Unranked commanders stay visible (no `#`; progress chip). Memo: `critique/decisions/vtsr-c-ladder-eligibility.md`. Do not retune the mins to chase a name. No `PIPELINE_VERSION` / `ELO_SCHEMA_VERSION` / `match.schema_version` bump.

**v3 — external community duels (F9bomber ledger; `CMDR_ELO_SCHEMA_VERSION 2 → 3`).** The ladder's history additionally walks the pre-gated community duels from `data/external/f9_ledger.json` (one-shot import of F9bomber's hand-kept match spreadsheet by the standalone `scripts/import_f9_ledger.py`; decision memo `critique/decisions/f9-external-duels.md`; ~505 duels, 2025 →). `compute_commander_elo(all_match_data, elo_history, external_duels=None, external_overlap_ids=None)`:

- **Merged chronological walk** — telemetry entries keep exact history order via sort key `(day, 0, history_index)`; externals interleave at `(day, 1, sheet_row)` (day precision — F9 logs calendar days; same-day externals sort AFTER telemetry; documented approximation, bounded by K).
- **Scoring** — outcome-pure S ∈ {1, 0} from `winner_side` at `k_factor(games) · CMDR_K_EXTERNAL_SCALE` (`1.0` = full K; operator ratified the outcomes as definitive — 28/28 agreement with telemetry on Steam64-paired overlapping games). Externals bump `games` (K decay + provisional math) and the headline W-L-D; `rated_match_count` stays telemetry-only, externals counted in top-level `external_duels_rated` + per-rating `duels_external`.
- **Team handicap** — a running `vtsr_t_now` snapshot (per-player VTSR-T `after`, folded from every telemetry history entry as the walk passes it) supplies mean thug strength over each side's **resolved** thugs; a side with none → `None` → `expected_score` zeroes the term. Pre-corpus duels see an empty snapshot → handicap 0, structurally identical to our own earliest telemetry matches.
- **Runtime overlap guard** — an external is skipped (WARN + `external_skipped_overlap_runtime`) when a corpus match within ±1 day carries the same commander-Steam64 pair and ≥ 60% of the external's resolved participants, EXCEPT matches already consumed by the importer's one-to-one pairing (`external_overlap_ids` from the ledger's `overlaps[]`) — a consumed match must not also block a sibling rematch row. Protects future binpb backfills from double-counting.
- **Provenance** — external duels carry `{match_id: "", source: "f9", external_row, map, decided_by: "external", performance: {available: false}}`; telemetry duels now carry `source: "telemetry"`. Per-rating `last_match_id` / `peak_at` use the `f9:<row>` sentinel. `external_provider {name, url}` powers the UI credit line ("Includes N community match records from F9bomber"). Pre-v3 `vtsr_c` / `peak_vtsr_c` are **not comparable** (re-rate signal).
- **VTSR-T is not rated from the ledger** — `scripts/elo.py` does not score F9 rows. It *does* pass gated `duels[]` into `compute_activity` so F9 appearances (commanders + thugs) refresh the display-only inactivity clocks; `elo_history.json` stays byte-identical. Gate: `_investigation/check_f9_ledger.py` (funnel partition, duel shape, alias validity, forbidden-consumer scan against `f9_ledger` / `f9_community` / `f9stats` string refs, ladder cross-checks).
- **Dual-recording dedup** — same-id dual recordings (two submitters' files landing on one match id) produce two `elo_history` entries; the telemetry walk rates the FIRST and skips the rest (`matches_skipped_duplicate_recording` counter) so one physical game is exactly one duel. **Batch adjudication** — the 50 unclear ledger-overlap matches passing the pre-registered six-gate rule (jaccard 1.0, duration ≤ 120s, two-sided pairing uniqueness) were auto-adjudicated by the one-shot `scripts/apply_f9_adjudications.py`; see the decision memo's addendum.
...
**Bands** key off the FAVORITE's probability and use the community vernacular for a lopsided lobby. The band label is the **one** status string, and it renders in exactly one place: the in-track text of `meterHtml()`. There is no separate band pill beside a caller's headline (it printed the same status twice) and no shorter/longer variant of the text (the old `compact` mode and the `defaultStatusText` / `shortStatusText` exports are gone). Which side is disadvantaged is carried by the surrounding copy — the caller's own `Team N favored X%` headline, the meter's `Team N disadv` end labels, and on Tools the `Disadvantaged` team-header badge (still keyed on `DISADVANTAGE_PROB`):
...
**What-if lobby.** The card header's **What-if** control opens Tools in a new tab with this match's rated lobby already placed. **As played** links to `tools/index.html?from=<match id>&ratings=then#vt-tools-balonce`; **Today** uses `ratings=now`. Tools (`js/tools/match-seed.js`) loads the match plus `elo_history.json` and `elo_commander_history.json`, sets roster mode to Manual, and seats each player on their real team in their real role (commander or thug). The roster is the rated set only: rows with a pre-match `before` on that match's `deltas` (or `shadow.deltas` when the match was cancelled). Camera-pod, partial, and idle rows are left out so As played opens on the same people as the card. As played freezes VTSR-T at that `before` and VTSR-C at the duel's commander `before` (otherwise the last prior duel `after`, else the anchor). Today uses the resolver's current ratings. The same two pills on the Team Balonce card flip the era without reloading and without clearing team swaps; the reset icon restores the match's teams and commanders. Refresh replays the URL. **Reset all** strips `from` and `ratings`. Display-only — nothing is written to localStorage, and neither basis feeds a rating.
...
| `js/player.js` | Single bundle that dispatches on URL params (`?p`, `?slug`, `?compare`, none). Renders all seven single-player tabs (Overview · Rating · Commander · Axes · Highlights · Rivals · Loadout) plus the compare view plus the directory. Self-contained — no shared state with the dashboard. |
...
**Single profile** (pre-gen stub or `player/index.html?p=<steam64>` / `?slug=<slug>`): seven tabs. The address bar tracks the open pill: Overview omits `tab`, and the others are `?tab=rating|commander|axes|highlights|rivals|loadout` (a redundant `overview` or unknown slug is stripped). Pill clicks `pushState`, so Back returns to the previous pill. The same query works on `/player/<slug>/`.

Directory filters are shareable when they differ from the defaults: `q`, `tier` (CSV), `role` (`commander`|`thug`; `any` omitted), `activity` (`any`|`veteran`|`pillar`; `active` omitted), `sort` (`vtsr-desc` omitted). Compare adds `x=matches` when the progression chart is not by date. Search commits push once, then replace while the field stays focused.

| Tab | Contents |
|---|---|
| Overview | Hero card (name, tier, VTSR-T big, peak chip, sparkline, corpus rank). Career snapshot stats. 8-axis radar (`renderVtsrRadar` in `js/player.js` over `rating.axis_means` — the VTSR-T axes, including PvE Share) with a dashed corpus-median ghost. Strengths/weaknesses ranking panel. Coaching cards (static per-axis copy dict triggered when z<median) with +0.5σ ΔVTSR quick-wins projection. The All Matches Career Radar stays the descriptive `renderPlayerRadar(mode='career')` spider in `js/charts-radar.js`. |
...
| Commander | VTSR-C rating, ladder rank, corpus W-L-D, time-series, vs-the-call and squad-gap lines, opposing commanders, favorite maps, faction when commanding, rated duel log, and v4 economy tiles (recorded, not scored). Empty state when the player has no commanded games and no rated duels. |
...
| Rivals | Top-10 two-way damage rivals, walked from each match's `rivalry_matrix`. |
...
The same push / popstate contract is used on the other standalone pages. Map directory: `q`, `pools`, `size`, `tag`, `played`, `author`, `sort` (`played-desc` omitted); `?file=` is not rewritten to a stub. Models directory: `q`, `faction` (omit `ISDF`; `all` when unfiltered), `cat` (omit `Building,Vehicle`; `all` when empty), `sort` (omit `name`); opening a model keeps those params. LEGO directory: `q`, `faction` (omit `all`), `sort` (omit `name`); a model opens on HQ, so `view` is omitted for HQ and set to `standard` or `photos` otherwise. Tools randomizer pills: `?tab=coinflip|maproll` (`shitwheel` omitted). ODF browser: `?cat=` when the sidebar category is not Vehicle and no ODF or compare is selected. Build trees: `?faction=hadean|scion` (`isdf` omitted), and Back restores the faction. ELO pills push `?tab=` the same way the player pills do (`vtsr-t` / `leaderboard` stay omitted).
...
The Lobby Tools rollout **deleted** the elaborate LIVE pill widget from every non-dashboard shell. The dashboard (`index.html`) keeps a compact `#vt-active-game` widget (LIVE pill + Join + GameWatch) whose click-through `#gamewatch-modal` iframes `/gw/?embed=1`. Full live-session interaction (roster, lock, ignore) lives at `/tools`.

- `js/active-game-indicator.js` is **dashboard-only**: `init()` early-returns unless `#vt-active-game` exists. It polls every 60s (presence-only; no fast path; 60s floor while the tab is hidden; 120s error cap), filters by `known-hosts.json`, renders the LIVE pill, and flips `data-vt-tools-live="0|1"` on the dashboard `[data-vt-tools-link]`.
- Game Watch (`js/gw/poller.js`) polls at 60s idle / 15s when an of-interest INGAME/PREGAME lobby is live (15s only while visible; 60s hidden floor; 120s error cap). `/gw/` does not load the indicator.
- Tools live-session uses the same 60s / 15s / 60s-hidden / 120s-error cadence. Ignore-live still pauses polling entirely.
...
When the telemetry shortlist is empty, the same scan tries non-overlap duels in `data/external/f9_ledger.json` (commanders, map, F9's calendar date ±1 day, F9bomber on the roster). A hit also needs the last HUD clock within 30 seconds of the sheet's `duration_sec` and a passing scoreboard `identity_gate` on the commander and thug names. One such duel writes `data/external/f9_video_links.json`, keyed by ledger row: video id, URL, clock, duration delta, and matched names. No segments and no `match_videos.json` row — a ledger-only game has no tick stream, so there is no kill-feed `&t=` link. Overlap rows stay on the telemetry path.
