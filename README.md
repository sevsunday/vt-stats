...
- **VTSR-C Commander Ladder (experimental)** — a separate win/loss ELO for commanders built from verified match outcomes, with a team-strength handicap so stacked thug teams don't inflate ratings. Ladder + explainers on the ELO page, top-5 strip on the dashboard's Commanders tab, and a Commander Rivalries panel (top opponent, head-to-head records, nemesis/best-matchup) on player profiles.
