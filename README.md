...
- **Commander Economy telemetry (proto v4)** — matches recorded with the v4 collector carry per-tick team scrap banks and every producer order (queue / cancel / build). Surfaces: a per-match **Economy tab** (dual-axis scrap + pool chart, full build log with unit names + costs + scrap-status stamps, per-team production cards), three commander highlight cards (**Elon Musk** — scrap generated; **Conveyor Belt** — combat-ship value fielded; **First Upgrade** — race to the first live upgraded pool), an Economy Leaders card on the All Matches Commanders tab, and career economy stats on commander profiles. Income is measured flow-complete from full-rate bank deltas and decomposed into pool regen / loose collected / refunds; the outflow ledger reconciles to ≈ 0 unaccounted as an engine-anomaly detector.
