...
The hero banner above the tab strip is built by `renderBanner()` + `renderMapBannerFields()` in `js/app.js`. The banner is intentionally lean: it surfaces match-level facts (duration, players, submitter) and a clickable map thumbnail that opens the Map Info Modal (below) for everything else.
...
| `teams{n}.income_loose_opening` / `scrap_income_opening` | `number` | **v29.** The same full-rate loose amount and total income, summed only on ticks in the first 240s (`ECON_OPENING_WINDOW_SEC`). Opening slice only. The unscored VTSR-C `loose_share` axis uses the whole-match `income_loose / scrap_income` instead. Not part of the income partition. |
...
| `teams{n}.scavs_built` | `number` | **v24.** Mobile scavenger **BUILD completions** only (Scavenger / Collector / Harvester — chain terminals `scavenger` / `scavengerh`). Queues and cancels do not count; deployed extractors are Pools / Upgrades, not this. Display-only; **not** in `_extract_contribution` / career |
...
| `{n}.commander_row` | `object\|null` | **v24.** Sibling of `thugs[]` with the **same shape** (`name` / `steam64` / `slot` / `ships_lost` / `ship_value_lost` / `pilot_sec` / `at_base_pilot_sec` / `reship_median_sec` / `reship_spells`), built from the side's `is_commander` leaderboard row. `null` when the side has no commander row. **Not folded into team totals** — `Σ thugs[] == thug_ships_lost` stays a thug-only invariant. Median re-ship time stays a thug-pooled median. Display-only; not in contributions. Economy card always shows the row (even at 0) with a muted `cmdr` chip |

The Economy card's per-player ship list is **not a stored field**. Under each thug (and the commander) who lost ships, the card joins `leaderboard[].per_ship_combat` on the full match — never the player-filtered view — and lists deaths whose stem is in `data/combat_ship_odfs.json`, skipping pilot ODFs (`user_m`). That is the same filter as `ships_lost`, so the line adds up to the count on the row. Scavengers, tugs, service pods and camera pods stay off it. Names come from `odf_map`. One ellipsized continuation row per player (`thug-lost-ships-${i}` / `cmdr-lost-ships`); the tooltip carries the unclipped list.
...
perturbs the whole block and requires byte-identical VTSR-T +
VTSR-C output).
...
| `facts.cast[]` | `object[]` | ≤4 dramatis personae `{role, name, steam64, team, ...role fields}`. Roles in priority order: `decisive_killer`, `extractor_hunter`, `heaviest_attrition`, `top_damage` — each ranks its candidates and takes the best player not already cast. Emitted for the template gate; The Story tab does not render the chips (Overview Match Highlights already covers those standouts) |
...
| Values | Resolved display name via `prettify_odf` — (1) weapons via `build_weapon_name_resolver` (the `Weapon.*` ordName / objectClass / leaderName / explosion chain, then the **child-ODF reverse map** below), (2) any other game object via `build_unit_name_resolver` (`GameObjectClass.unitName` across `Vehicle`, `Building`, `Powerup`, `Pilot`, `Ordnance`), (3) title-cased form of the raw stem as a last-resort fallback for ODFs the DB does not recognize. |
| Child-ODF reverse map | **`PIPELINE_VERSION` 49.** Mines, dispensed payloads, charge levels and impact explosions deal damage under their own ODF, so the wire reports `xseekvsrxpl` where the player fired a `gseekervsr` — 88 such stems carried ~15% of corpus weapon damage as raw text. `build_child_odf_reverse_map()` walks every named weapon's reference graph (the `xpl*` family, `explosionName`, `payloadName`, `launchOrd`, `ordName<N>`, `altName`, nested `Expl*.ExplosionClass.classLabel`) up to 3 hops and claims the stems it reaches, skipping any stem that has its own `unitName` or is itself a named weapon. Shared assets are decided by a deterministic chain (fewest hops → stem-prefix similarity → majority → alphabetical); the six generic wreck explosions (`xvehxpl`, `xcarxpl`, `xsgnxpl`, `xpwrxpl`, `_e` variants, `kamixpl`) get curated labels from `GENERIC_EXPLOSION_NAMES` instead, checked first. Display-only — explosion stems carry zero shots/hits, so no VTSR-T axis moves. Full derivation: `DEVELOPER_GUIDE.md` §4. |
| Disambiguation | Same-name collisions inside a match (e.g. multiple "Pulse" weapons or stock + `_vsr` "Scavenger") resolve as `Name (raw_stem)` via the shared `disambiguate_names` helper. The reverse map renames but never merges, so a child sharing its parent's name picks up the suffix on both rows (`FAF Msl (fafmsl_c)` + `FAF Msl (xfafmsl)`). |
...
Per-match award catalog. A fixed slate of **12 always-on cards** ("The Bully", "Grim Reaper", "Bullet Sponge", "The Hustler", "Sharpshooter", "Gunner", "Puppeteer", "Frenemies", "Roadrunner", "Pod Goblin", "Chris Kyle", "The Locksmith"), plus **four proto-v4 commander-economy cards** that self-omit pre-v4: **Elon Musk** (`the_tycoon`, scrap generated), **Loose Collector** (`loose_collector`, loose scrap collected), **Conveyor Belt** (`war_machine`, combat-ship value fielded), **First Upgrade** (`first_upgrade`, race to the first live upgraded pool). Each card is emitted unconditionally as long as its data gate passes; cards whose underlying data is unavailable (positioning / pickups / T-key gates, missing snipes, no resource/build telemetry, etc.) are simply omitted. Pre-computed in `scripts/process_stats.py` `compute_highlights()` from already-built per-match blocks (`leaderboard`, `top_rivalries`, `pickups`, `powerup_destructions`, `snipes`, `positioning`, `economy`, `builds`). **Match-global, always-unfiltered** — the dashboard reads `currentData.highlights` and never narrows under `filterState`. Display labels for `the_tycoon` / `war_machine` are "Elon Musk" / "Conveyor Belt" (JSON category keys are stable).
...
| `cards` | `array` | Cards in the canonical render order (see catalog below). May be shorter than 12 (or 16 on v4) — failed gates simply drop entries. Empty arrays are valid; the UI hides the section. |

Each entry in `cards[]`:

| Field | Type | Description |
|---|---|---|
| `category` | `string` | Stable id (snake_case). One of `the_bully`, `the_grim_reaper`, `bullet_sponge`, `the_hustler`, `sharpshooter`, `gunner`, `puppeteer`, `frenemies`, `roadrunner`, `crate_pod_goblin`, `chris_kyle`, `the_locksmith`, plus v4 `the_tycoon`, `loose_collector`, `war_machine`, `first_upgrade`. |
...
| `value_format` | `string` | Renderer hint: `"damage"`, `"count"`, `"score"`, `"ratio"`, `"kd"`, `"percent"`, `"accuracy"`, `"distance"`, `"scrap"`, `"duration"`. Drives [`formatHighlightValue()`](../js/app.js) — locale-formatted integer for `damage`/`count`/`score`/`distance`/`scrap`; two decimals for `ratio` and `kd`; `(value * 100).toFixed(1) + '%'` for `percent`/`accuracy`; `Nm:Ss` (e.g. `1m:30s`) for `duration`. `kd` is functionally identical to `ratio` at the value-formatting layer; the separate enum value tells the renderer to render the breakdown line as `{kills}K / {deaths}D` (with `(perfect)` instead of a number when `deaths == 0`). |
...
| `delta_pct` | `number \| null` | `(winner_value − runner_up_value) / runner_up_value` for higher-is-better cards, or `(runner_up_value − winner_value) / runner_up_value` for lower-is-better (`first_upgrade`), rounded to 3 decimals. `null` when there is no runner-up or the runner-up value is non-positive. |
...
| 13 | `the_tycoon` — Elon Musk | `bi-cash-stack` | `max economy.teams{n}.scrap_income` (winner = that team's commander) | `match.has_resource_data` | team slot (1 before 2) |
| 14 | `loose_collector` — Loose Collector | `bi-box-arrow-in-down` | `max economy.teams{n}.income_loose` (winner = that team's commander) | `match.has_resource_data` and at least one side collected loose | team slot (1 before 2) |
| 15 | `war_machine` — Conveyor Belt | `bi-gear-wide-connected` | `max builds.teams{n}.combat_ship_value` (winner = that team's commander) | `match.has_build_data` | team slot (1 before 2) |
| 16 | `first_upgrade` — First Upgrade | `bi-arrow-up-circle` | `min economy.teams{n}.time_to_first_upgrade_sec` (first tick where `upgrade_count >= 1`; lower is better) | `match.has_resource_data` and at least one side upgraded | team slot (1 before 2) |
...
| `the_tycoon` | `income_regen`, `income_loose`, `loose_collections`, `loose_share` | Income split of the winning commander's team. Renderer shows `{income_regen} scrap · {income_loose} loose`. |
| `loose_collector` | *(empty)* | Headline is the loose amount; no breakdown sub-line (piece count stays out of display per the loose terminology contract). |
| `war_machine` | `ships_built`, `units_built`, `builds_per_min` | Combat-ship count + production rate of the winning commander's team. |
| `first_upgrade` | `upgrade_name` (string \| null) | Display name of the winner's first `*scup*` build-feed order (`Extractor+` / `Refinery+`) when build data exists. |
...
| **Match Highlights** | Per-match award catalog rendered below the Faction Scoreboard. A fixed 12-card slate (The Bully, Grim Reaper, Bullet Sponge, Hustler, Sharpshooter, Gunner, Puppeteer, Frenemies, Roadrunner, Pod Goblin, Chris Kyle, Locksmith) plus four proto-v4 commander cards (Elon Musk, Loose Collector, Conveyor Belt, First Upgrade). Cards self-omit when their data gates fail (positioning / pickups / T-key gates, no snipes, no resource/build telemetry, etc.) — the renderer reflows around the gap | Per-match `highlights.cards[]` produced by `compute_highlights()` in `scripts/process_stats.py` | Sourced from already-built per-match blocks (`leaderboard`, `top_rivalries`, `pickups`, `powerup_destructions`, `snipes`, `positioning`, `economy`, `builds`). Match-global, always-unfiltered (read directly from `currentData.highlights`, not the filtered view). New top-level field added at `meta.schema_version = 2`; pre-v2 matches have no block and the section is hidden. See §5 `highlights` and the canonical card catalog there |
...
>
> **v2.7 — retired.** A parallel `elo_*_thugs_only.json` pair plus an `[All matches | Thug-only]` toggle scored VTSR-T after dropping commander rows. Product and emit are gone: one canonical `compute_elo()` pass, alt files deleted, `exclude_commanders` removed from `scripts/elo.py`, `/elo` and the dashboard always read `elo_current.json` / `elo_history.json`. Boot nukes the orphaned `vt.elo_mode` localStorage key. Canonical files may still carry leftover `excludes_commanders: false` / `rows_excluded_commander_mode: 0` until the next normal `process_stats.py` run. The v2.7-era lobby-index alignment fix in `compute_elo` (filter `lobby` with the same campod/low-activity predicate as `compute_performance_index`) remains. No `PIPELINE_VERSION` / `ELO_SCHEMA_VERSION` / `match.schema_version` bump for this retirement.
...
**v2 (proto v4 overhaul) adds an economy-performance composite, structurally INERT behind `alpha_c = 1.0`** — formulas frozen in [critique/decisions/vtsr-c-v2-composite.md](../critique/decisions/vtsr-c-v2-composite.md) BEFORE any telemetry corpus accumulated. Scored axes per telemetry duel (both `has_resource_data` AND `has_build_data` required), amended 2026-09-23: `pool_tempo` (seconds sooner to 3 pools inside the first 240s; later or never is censored at 240s), `combat_conversion` (opening combat-ship BUILD scrap / opening 1 Hz income), `regen_tempo` (opening share of alive-recycler samples in the red band). `replacement_ratio` (the old ships-per-death formula), `upgrade_share` (the old upgrade snapshot), and `loose_share` (`income_loose / max(scrap_income, 1)`, whole match) stay in the audit at weight 0. Normalization is the **within-match differential** (n=2 makes lobby z-scores degenerate): `z = clip(diff / shrunk_std(axis), −2, 2) / 2` with a rolling per-axis RMS (seed prior + shrinkage 10 pseudo-duels, snapshot-before-duel / update-after); `P₁ = Σw·z / Σw`, `P₂ = −P₁`. Blend `S' = alpha_c·S + (1−alpha_c)·(P+1)/2` applies only on telemetry duels — the code branches on `alpha_c < 1.0` so the inert path never routes through blend arithmetic (golden byte-identity gate: `_investigation/golden_vtsrc_v2.py`). Duels without telemetry score outcome-pure S at ANY alpha_c (ratified fallback policy). W/L/D records always tally the RAW outcome. New v2 fields: top-level `alpha_c`, `econ_weights`, `econ_std_prior`, `econ_std_shrinkage`, `thug_supply_cap`, `econ_std_observed {axis: {count, std}}`; per-rating `duels_with_telemetry`; per-duel `performance {available, p, axes{axis: {diff, std, z, weight}}}` (team-1 perspective; side 2 = negation) + per-side `score_blend {alpha_c, s_raw, s_blended}`. Promote rule (2026-09-23 amendment; the 2026-09-04 rule is retired): the pre-amendment corpus is discovery only. Flip below 1.0 only on duels dated after 2026-09-23, and only when that sample has ≥ 25 duels, ≥ 2 of 3 scored axes have a Wilson lower bound > 0.55, the close subset (n ≥ 15) is not below 0.50, and log-loss improves by ≥ 0.01 without worse accuracy. `CMDR_ELO_SCHEMA_VERSION` 6; ratings comparable with schema 5.
...
**`validation_summary.json` additions (validator v1.5 / `report.json` schema 6, additive; summary `schema_version` stays 1):** optional keys the Does-it-work tab reads when present and ignores when absent. `latest.clean_win_accuracy_recent` — last 30 determined matches, hard-max headline `{window_n, n, since_date, n_correct, accuracy, accuracy_ci, aggregations.{mean,hard_max,softmax_max}}`. `latest.mean_vtsr` / `latest.rating_spread_std` — pool drift, no UI. `latest_detail.accuracy_timeline` — `[{date, correct_hard_max}]` in match order; the page draws cumulative and rolling-30 itself. `latest_detail.winner_funnel.recent` — `{window_n, rated, determined, since_date}` over the last 30 rated matches.
