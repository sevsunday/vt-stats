...
player profile links, Tools resolution, the per-match VTSR-T Δ column —
attributes the source account's gameplay to the target. Unlike
ACCOUNT_REROUTES (the shared-PC mechanism in scripts/process_stats.py),
a silent alias:

  - matches on Steam64, not on in-game nick patterns;
  - keeps the SOURCE account's display name on every per-match surface
    (leaderboard, kill feed, picker roster, highlights, storyline cast) —
    the merge is never visible in the UI;
...
pipeline ALWAYS aborts with a clear error so the operator sees the
situation — this is deliberately stricter than outcome adjudication's
defer-and-continue posture.
